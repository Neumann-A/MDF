...
In addition to those abbreviations for dataset dimensions MDF specifies a naming schema for Boolean flags. Flags are either prefixed with \inlvar{is} or \inlvar{has} depending which consequence it has on the interpretation of the data. The \inlvar{has} prefix is reserved for flags which describe the ordering of data within a dataset whereas flags prefixed with \inlvar{is} are data descriptive. This means that \inlvar{is} flags, in general, define a narrower context for the interpretation of the data.  In contrast to that \inlvar{has} flags do not narrow the context but influence the ordering of the data within a dataset. This specification will only define a few \inlvar{has} flags because data ordering - in most cases -  is implicitly defined by the dimensions of the specified datasets and thus deemed unnecessary. 
...
\inlvar{hasFastFrameAxis} & \inltab{Int8} & 1 & no & Flag, if the frame dimension $N$ has been moved to the last dimension\\ \hline
\inlvar{hasFramePermution} & \inltab{Int8} & 1 & no & Flag, if the order of frames has been changed, see \inlvar{framePermutation} \\ \hline 
